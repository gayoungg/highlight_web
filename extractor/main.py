...
def extraction(f, fname, length=30, save_score=True, save_thumbnail=True, save_wav=True):
    with tf.Session() as sess:
        ...
        if save_score:
            np.save('{}_score.npy'.format(fname), attn_score)
        ...
        if save_thumbnail:
            np.save('{}_highlight.npy'.format(fname), highlight)

        if save_wav:
            librosa.output.write_wav('media/music/{}_output.wav'.format(fname), audio[highlight[0]*22050:highlight[1]*22050],
                                     22050)
